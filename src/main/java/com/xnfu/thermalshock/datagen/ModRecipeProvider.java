...
public class ModRecipeProvider extends RecipeProvider {
    ...
    protected void buildRecipes(RecipeOutput output) {
        ...
        // Path: recipes/overheating/
        // =================================================================
        
        // 1.1 Item -> Item (基础金属加工)
        // 铁粒 -> 铁粒 (处理测试: 3个输入 -> 1个输出)
        buildOverheating(output, "iron_nuggets_processing", 
        ...
        buildOverheating(output, "log_to_charcoal", 
        ...
        // 1.3 Block -> Block (转化)
        // 湿海绵 -> 干海绵
        buildOverheating(output, "wet_sponge_drying",
        ...
        // 这里用 Bucket 表示流体原料
        buildOverheating(output, "lava_to_obsidian",
        ...
        // 2.1 Block -> Item (粉碎)
        // 泥土 -> 粘土球
        buildShock(output, "dirt_to_clay",
                Ingredient.of(Blocks.DIRT), 1,
                new ItemStack(Items.CLAY_BALL, 4),
                100, 10, 150, true);

        // 圆石 -> 沙砾
        buildShock(output, "cobble_to_gravel",
        ...
        buildShock(output, "water_to_ice",
        ...
        // 2.3 Item -> Item (精密加工)
        // 玻璃 -> 玻璃板 (利用热冲击切割)
        buildShock(output, "glass_cutting",
        ...
        // Path: recipes/extraction/
        // =================================================================
        // 注意: 这里需要为每种可能的团块生成对应的提取配方
        
        // 铁锭团块提取
        buildClumpProcessing(output, "extract_iron_ingot",
                new ItemStack(Items.IRON_INGOT), 200, 1000);

        // 金锭团块提取
        buildClumpProcessing(output, "extract_gold_ingot",
                new ItemStack(Items.GOLD_INGOT), 500, 2000);

        // 铁粒团块提取
        buildClumpProcessing(output, "extract_iron_nugget",
        ...
        buildFuel(output, "fuel_coal", Ingredient.of(Items.COAL), 1600, 200);
        buildFuel(output, "fuel_blaze_rod", Ingredient.of(Items.BLAZE_ROD), 2400, 500);
        buildFuel(output, "fuel_lava_bucket", Ingredient.of(Items.LAVA_BUCKET), 20000, 1000);
        buildFuel(output, "fuel_magma_cream", Ingredient.of(Items.MAGMA_CREAM), 1200, 300);

        // 5.2 Freezers (Negative Heat)
        buildFuel(output, "coolant_snowball", Ingredient.of(Items.SNOWBALL), 200, -50);
        buildFuel(output, "coolant_ice", Ingredient.of(Items.ICE), 1200, -200);
        buildFuel(output, "coolant_packed_ice", Ingredient.of(Blocks.PACKED_ICE), 4800, -350);
        buildFuel(output, "coolant_blue_ice", Ingredient.of(Blocks.BLUE_ICE), 19200, -500);

        // =================================================================
        // 6. 热力转换器 (Thermal Converter)
        // Path: recipes/converter/
        // =================================================================
        ...
        ThermalConverterRecipeBuilder.create()
                ...
                .save(output, loc("converter/melt_cobble"));
        ...
        ThermalConverterRecipeBuilder.create()
                ...
                .maxHeat(0).time(100)
                .save(output, loc("converter/freeze_water"));
        ...
        ThermalConverterRecipeBuilder.create()
                ...
                .minHeat(50).time(60)
                .save(output, loc("converter/dry_sponge"));
        ...
        ThermalConverterRecipeBuilder.create()
                ...
                .minHeat(200).time(80)
                .save(output, loc("converter/sift_gravel"));
    }

    // === Helpers ===

    private ResourceLocation loc(String path) {
        return ResourceLocation.fromNamespaceAndPath(ThermalShock.MODID, path);
    }
