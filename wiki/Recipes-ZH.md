...
| `min_heat` | 整数 | 是 | 启动所需的最小热量速率 (H/t)。 |
| `heat_cost` | 整数 | 是 | 加工该配方所需的累计总热量 (H)。 |
...
```json
{
  ...
  "ingredients": [
    ...
    { "type": "block", "value": { "block": "minecraft:magma_block" } }
  ],
...
| 属性 | 类型 | 必须 | 说明 |
| :--- | :--- | :--- | :--- |
| `ingredients` | 数组 | 是 | 输入列表（最多 3 物品 + 3 方块）。支持流体方块。 |
| `result` | 对象 | 是 | 产出物品。 |
| `min_hot` | 整数 | 是 | 模拟室内所需的最小高温热量值 (H)。 |
| `max_cold` | 整数 | 是 | 模拟室内允许的最大低温热量值 (H，通常为负数)。 |
...
| 属性 | 类型 | 必须 | 说明 |
| :--- | :--- | :--- | :--- |
| `ingredients` | 数组 | 是 | 输入列表（最多 4 项）。必须包含 `thermalshock:material_clump`。 |
| `target_result` | 对象 | 是 | 被注入团块内部的产物信息（提取时产出什么）。 |
| `min_hot` | 整数 | 是 | 填充所需的最小高温。 |
| `max_cold` | 整数 | 是 | 填充所需的最大低温。 |
| `delta` | 整数 | 是 | 填充所需的最小温差。 |
| `clump_min_heat` | 整数 | 是 | **写入团块的数据**：未来提取该团块所需的最小热量。 |
| `clump_heat_cost` | 整数 | 是 | **写入团块的数据**：未来提取该团块所需的累计热量。 |
...
从已填充的团块中还原物品。**输入中必须包含已填充数据的团块物品。**

### 属性列表
| 属性 | 类型 | 必须 | 说明 |
| :--- | :--- | :--- | :--- |
| `ingredients` | 数组 | 是 | 输入列表（最多 4 项）。必须包含带数据的团块。 |
| `target_content` | 对象 | 是 | 只有内部产物数据与此项匹配的团块才会被处理。 |
| `min_heat` | 整数 | 是 | 提取所需的最小热量速率。 |
| `heat_cost` | 整数 | 是 | 提取所需的累计热量。 |
...
| `burn_time` | 整数 | 是 | 燃烧持续时间（以游戏刻为单位）。 |
| `heat_rate` | 整数 | 是 | 产生的热量速率。正数为加热，负数为冷却。 |
...
| 属性 | 类型 | 必须 | 说明 |
| :--- | :--- | :--- | :--- |
| `item_inputs` | 数组 | 否 | 输入物品列表。每项含 `ingredient`, `count`, `consume_chance`。 |
| `fluid_inputs` | 数组 | 否 | 输入流体列表。每项含 `fluid` (ID和数量), `consume_chance`。 |
| `item_outputs` | 数组 | 否 | 产出物品列表。每项含 `item`, `chance` (0.0~1.0)。 |
| `fluid_outputs` | 数组 | 否 | 产出流体列表。每项含 `fluid`, `chance`。 |
| `process_time` | 整数 | 是 | 加工耗时 (ticks)。 |
| `min_heat` | 整数 | 否 | 允许工作的最小热量 (默认负无穷)。 |
| `max_heat` | 整数 | 否 | 允许工作的最大热量 (默认正无穷)。 |
